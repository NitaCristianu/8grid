...
export default function Actions() {
    ...
    const remove = () => {
        ...
        // var data: copies = {
        //     points : points_clone,
        //     calcs : points_calc_clone,
        //     segments : segments_clone
        // }
        // console.log('before: ' , data);
        // selected.forEach(id=>{
        //     console.log('removing : ', id);
        //     var index = data.segments.findIndex(p=>p.id == id);
        //     if (index) {data = RemoveSegment(id, data); return;}
        //     data = RemovePoint(id, data);
        // })
    ...
    const modifyProperty = useCallback((prop: {
        ...
        selected.forEach(id => {
            ...
            finds.forEach(find => {
                ...
                if (prop.type == "char" && event.target.value.toUpperCase() != event.target.value.toLowerCase()) {
                    find.table[find.index][prop.id] = event.target.value.toUpperCase().charAt(event.target.value.length - 1);
                    return;
                }
                if (prop.type == "text"){
                    console.log(find.table[find.index][prop.id]);
        ...
        set_points(points_clone);
        set_segments(segments_clone);
    }, [points, segments, props, selected]);
    ...
    }, [selected, points, offset, mode, props])
