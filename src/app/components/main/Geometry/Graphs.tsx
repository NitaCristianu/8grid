...
import { ACCENT, BACKGROUND, GRID_POSITION, HOVERING_GRAPHS, HOVERING_LABELS, MODE, MOUSE_PRIORITY, SECONDARY, SELECTED_GRAPH, VARIABLES, vec2D } from "@/app/data/globals";
...
const GraphComponent = React.memo((props: GraphProps) => {
    ...
    const data: { x: number[], y: number[], name: string, marker: { color: string | rgb | undefined, width: number }, mode: string }[] = [];
    props.data.functions.forEach(func => {
        ...
        data.push({ x: x_data, y: y_data, name: func.expression, mode: "lines", marker: { color: func.color, width: 9 } });
    })

    const inEdit = selected == props.data.id;
    const Mdown = (event: MouseEvent) => {
    ...
    }, [dragging, mode, selected, mpos, prevMpos, hovered_labels])
    ...
    return (<div style={{ zIndex: 2, position: "fixed", left: 0, top: 0 }}>
        {Graphs.map(data => <GraphComponent
            ...
            removeFunc={() => {
                const clone = [...Graphs];
                set_selected("");
                if (hovering_graphs.includes(data.id)) {
                    setHoveringGraphs(prev => prev.splice(prev.findIndex(g => g == data.id), 1));
                }
