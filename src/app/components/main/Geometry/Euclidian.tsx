"use client"
import { Anchors, ePoints_Calc_data, ePoints_data, eSegments_data } from "@/app/data/elements";
import { CAN_SELECT, HOVERING_GRAPHS, HOVERING_LABELS, MODE, MOUSE_PRIORITY, POINT_RADIUS, SEGMENT_WIDTH, SELECTED, SELECT_RECT, VARIABLES, vec2D } from "@/app/data/globals";
...
import { Anchor, ePoint, ePoints_Calc, eSegment, variable } from "@/app/data/props";
...
function DrawAnchors(ctx : CanvasRenderingContext2D, anchors : Anchor[], offset : vec2D){
    ctx.fillStyle = "rgba(155, 241, 136,)"
...
export default function Euclidian() {
    ...
    useEffect(() => {
        ...
        const MouseMove = (event: MouseEvent) => {
            ...
            if (mode != "selection") { set_select_rect([]); }
            if (!can_select) { set_select_rect([]); return; }
            if (!can_select) { setSelecting(false); set_select_rect([]); return; }
            SetMpos({ 'x': event.clientX, 'y': event.clientY });
            if (selecting && mode == "selection") {
    ...
    }, [selecting, offset, points, selected, mpos, mode, select_rect, segments, can_select])
    ...
    useEffect(() => {
        ...
        DrawAnchors(context, anchors, offset);
        DrawPoints(context, points, points_calc, variables, offset);
        if (selecting) DrawSelection(context, select_rect, offset);
    }, [points, size, offset, selected, select_rect, selecting, segments, ePoints_Calc_data, hovering_graphs, hovering_labels, variables, anchors])
